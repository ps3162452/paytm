...
public final class Detector {
    ...
    static final class Point {
        ...
        public String toString() {
            return NPStringFog.decode(new byte[]{89}, "e4cad1", false) + this.x + ' ' + this.y + '>';
        }
    }
    ...
    private ResultPoint[] getBullsEyeCorners(Point point) throws NotFoundException {
        ...
        while (this.nbCenterLayers < 9) {
            ...
            if (this.nbCenterLayers > 2) {
                float fDistance = (distance(firstDifferent4, firstDifferent) * this.nbCenterLayers) / (distance(point4, point3) * (this.nbCenterLayers + 2));
                if (fDistance < 0.75d || fDistance > 1.25d || !isWhiteOrBlackRectangle(firstDifferent, firstDifferent2, firstDifferent3, firstDifferent4)) {
                    break;
                }
            }
            z = !z;
        ...
        return expandSquare(new ResultPoint[]{new ResultPoint(point3.getX() + 0.5f, point3.getY() - 0.5f), new ResultPoint(point2.getX() + 0.5f, point2.getY() + 0.5f), new ResultPoint(point.getX() - 0.5f, point.getY() + 0.5f), new ResultPoint(point4.getX() - 0.5f, point4.getY() - 0.5f)}, (this.nbCenterLayers * 2) - 3, this.nbCenterLayers * 2);
    }
    ...
    private Point getMatrixCenter() {
        ...
        int iRound2 = MathUtils.round(((resultPoint2.getY() + (resultPoint.getY() + resultPoint4.getY())) + resultPoint3.getY()) / 4.0f);
        try {
            ...
            resultPoint5 = getFirstDifferent(new Point(iRound + 7, iRound2 - 7), false, 1, -1).toResultPoint();
            resultPoint6 = getFirstDifferent(new Point(iRound + 7, iRound2 + 7), false, 1, 1).toResultPoint();
            resultPoint7 = getFirstDifferent(new Point(iRound - 7, iRound2 + 7), false, -1, 1).toResultPoint();
            resultPoint8 = getFirstDifferent(new Point(iRound - 7, iRound2 - 7), false, -1, -1).toResultPoint();
        }
        return new Point(MathUtils.round((((resultPoint5.getX() + resultPoint8.getX()) + resultPoint6.getX()) + resultPoint7.getX()) / 4.0f), MathUtils.round(((resultPoint6.getY() + (resultPoint5.getY() + resultPoint8.getY())) + resultPoint7.getY()) / 4.0f));
    }
    ...
    private static int getRotation(int[] iArr, int i) throws NotFoundException {
        ...
        for (int i4 = 0; i4 < 4; i4++) {
            if (Integer.bitCount((((i2 & 1) << 11) + (i2 >> 1)) ^ EXPECTED_CORNER_BITS[i4]) <= 2) {
                return i4;
            }
        }
        throw NotFoundException.getNotFoundInstance();
