...
public class ShapePath {
    ...
    public void addArc(float f, float f2, float f3, float f4, float f5, float f6) {
        ...
        float f7 = f5 + f6;
        boolean z = f6 < 0.0f;
        float f8 = z ? (f5 + ANGLE_LEFT) % 360.0f : f5;
        if (z) {
            f7 = (f7 + ANGLE_LEFT) % 360.0f;
        }
        addShadowCompatOperation(arcShadowOperation, f8, f7);
        setEndX(((f + f3) * 0.5f) + (((f3 - f) / 2.0f) * ((float) Math.cos(Math.toRadians(f5 + f6)))));
        setEndY(((f2 + f4) * 0.5f) + (((f4 - f2) / 2.0f) * ((float) Math.sin(Math.toRadians(f5 + f6)))));
    }
