...
public class DrawerArrowDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        double d = fLerp;
        double dCos = Math.cos(fLerp3);
        Double.isNaN(d);
        float fRound2 = Math.round(d * dCos);
        double d2 = fLerp;
        double dSin = Math.sin(fLerp3);
        Double.isNaN(d2);
        ...
        canvas.translate(bounds.centerX(), (strokeWidth * 1.5f) + this.mBarGap + ((((int) ((fHeight - (3.0f * strokeWidth)) - (2.0f * r5))) / 4) * 2));
        if (this.mSpin) {
