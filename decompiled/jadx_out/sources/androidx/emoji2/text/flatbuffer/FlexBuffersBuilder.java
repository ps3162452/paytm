...
public class FlexBuffersBuilder {
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i) {
        ...
        this.keyComparator = new Comparator<Value>(this) { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i3 = value2.key;
                do {
                    b = this.this$0.bb.get(i2);
                    b2 = this.this$0.bb.get(i3);
                    if (b == 0) {
                        return b - b2;
                    }
                    i2++;
    ...
    private Value createKeyVector(int i, int i2) {
        int i3 = i;
        int iMax = Math.max(0, widthUInBits(i2));
        while (i3 < this.stack.size()) {
            int iMax2 = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i3).key, this.bb.writePosition(), i3 + 1));
            i3++;
            iMax = iMax2;
        }
    ...
    private Value createVector(int i, int i2, int i3, boolean z, boolean z2, Value value) {
        ...
        int i4;
        int typedVector;
        int iMax2 = Math.max(0, widthUInBits(i3));
        if (value != null) {
            iMax = Math.max(iMax2, value.elemWidth(this.bb.writePosition(), 0));
            i4 = 3;
        } else {
            iMax = iMax2;
            i4 = 1;
        ...
        while (i7 < this.stack.size()) {
            int iMax3 = Math.max(i6, this.stack.get(i7).elemWidth(this.bb.writePosition(), i7 + i4));
            if (z && i7 == i2) {
                i5 = this.stack.get(i7).type;
                if (!FlexBuffers.isTypedVectorElementType(i5)) {
                    throw new FlexBuffers.FlexBufferException(NPStringFog.decode(new byte[]{99, 64, 73, 1, 0, 110, 82, 90, 77, 11, 22, 24, 83, 86, 92, 23, 68, 86, 88, 77, 25, 23, 17, 72, 71, 86, 75, 16, 68, 76, 95, 80, 74, 68, 1, 84, 82, 84, 92, 10, 16, 24, 67, 64, 73, 1}, "799dd8", 959229293L));
                }
            }
            i7++;
            i5 = i5;
        ...
        for (int i8 = i2; i8 < this.stack.size(); i8++) {
            writeAny(this.stack.get(i8), iAlign);
        }
        if (!z) {
            while (i2 < this.stack.size()) {
                this.bb.put(this.stack.get(i2).storedPackedType(i6));
                i2++;
            }
        }
        if (value != null) {
            ...
            typedVector = FlexBuffers.toTypedVector(i5, i3);
        } else {
            typedVector = 10;
        }
        return new Value(i, typedVector, i6, iWritePosition);
    }
