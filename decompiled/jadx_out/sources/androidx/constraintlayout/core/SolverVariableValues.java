...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    private void displayHash() {
        for (int i = 0; i < this.HASH_SIZE; i++) {
            if (this.keys[i] != -1) {
                String str = hashCode() + NPStringFog.decode(new byte[]{24, 93, 88, 74, 80, 24, 99}, "859988", -1.767656847E9d) + i + NPStringFog.decode(new byte[]{106, 19, 4, 95, 67}, "739acb", -1465308365L);
                int i2 = this.keys[i];
                boolean z = false;
                while (!z) {
    ...
    public void display() {
        int i = this.mCount;
        System.out.print(NPStringFog.decode(new byte[]{26, 68}, "ad9693", HASH, false));
        for (int i2 = 0; i2 < i; i2++) {
            SolverVariable variable = getVariable(i2);
            if (variable != null) {
                System.out.print(variable + NPStringFog.decode(new byte[]{68, 89, 69}, "ddecba", false) + getVariableValue(i2) + " ");
            }
        }
        System.out.println(NPStringFog.decode(new byte[]{17, 74}, "17c4ae", -5.2559562E8f));
    }
    ...
    public String toString() {
        String str = hashCode() + NPStringFog.decode(new byte[]{24, 76, 66}, "87bddf", 2028446289L);
        int i = this.mCount;
        for (int i2 = 0; i2 < i; i2++) {
            SolverVariable variable = getVariable(i2);
            if (variable != null) {
                String str2 = str + variable + NPStringFog.decode(new byte[]{17, 10, 23}, "1775d0", false) + getVariableValue(i2) + " ";
                int iIndexOf = indexOf(variable);
                String str3 = str2 + NPStringFog.decode(new byte[]{99, 66, 95, 19}, "82e309", -5.92760933E8d);
                String str4 = (this.previous[iIndexOf] != -1 ? str3 + this.mCache.mIndexedVariables[this.variables[this.previous[iIndexOf]]] : str3 + NPStringFog.decode(new byte[]{91, 11, 88, 87}, "5d627b", HASH, HASH)) + NPStringFog.decode(new byte[]{30, 21, 11, 2, 24}, "25e885", -3584);
                str = (this.next[iIndexOf] != -1 ? str4 + this.mCache.mIndexedVariables[this.variables[this.next[iIndexOf]]] : str4 + NPStringFog.decode(new byte[]{12, 88, 11, 86}, "b7e380", -133130819L)) + NPStringFog.decode(new byte[]{104}, "588188", HASH, HASH);
            }
        }
        return str + NPStringFog.decode(new byte[]{67, 72}, "c5518c", -1.823670725E9d);
    }
