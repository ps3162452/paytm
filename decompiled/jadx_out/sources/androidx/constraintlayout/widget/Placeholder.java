...
public class Placeholder extends View {
    ...
    public void onDraw(Canvas canvas) {
        if (isInEditMode()) {
            ...
            paint.getTextBounds(NPStringFog.decode(new byte[]{15}, "08071d", true), 0, NPStringFog.decode(new byte[]{14}, "12a698", 1.4676708E9f).length(), rect);
            canvas.drawText(NPStringFog.decode(new byte[]{9}, "655baa", false), ((iWidth / 2.0f) - (rect.width() / 2.0f)) - rect.left, ((iHeight / 2.0f) + (rect.height() / 2.0f)) - rect.bottom, paint);
        }
    }
